...
def ReadinData(filename):
	...
	return lon, lat, lat_weight, hy_am, hy_bm, hy_ai, hy_bi, pres, temp
...
lon, lat, lat_weight, hy_am, hy_bm, hy_ai, hy_bi, pres, temp = ReadinData(files[0])
...
temp_all	= ma.masked_all((len(time_year), len(P_lev), len(lon)))
...
for year_i in range(int(np.min(time)), int(np.min(time))+len(time_year)):
	...
	for month_i in range(month_start, month_end+1):
		...
		lon, lat, lat_weight, hy_am, hy_bm, hy_ai, hy_bi, pres, temp	= ReadinData(filename)
		...
		for lat_i in range(len(lat)):
			for lon_i in range(len(lon)):
				...
				x, y		= P_3_log[:, lat_i, lon_i], temp[:, lat_i, lon_i]
				x_int		= P_lev_log[P_lev_log <= x[-1]]
				temp_int	= interp1d(x,y)(x_int)
	...
	for month_i in range(len(month_days)):
		...
		month_days_all[month_i]		= ma.masked_array(month_days_all[month_i], mask = temp_year[month_i].mask)
	...
	temp_all[year_i - int(np.min(time))]	= np.sum(temp_year * lat_weight, axis = 1)
...
fh = netcdf.Dataset(directory+'Atmosphere/Air_TEMP_meridional_mean_5S_5N_month_'+str(month_start)+'-'+str(month_end)+'_branch3800_year_'+str(year_start)+'_'+str(year_end)+'.nc', 'w')
...
fh.createVariable('T', float, ('time', 'lev', 'lon'), zlib=True)
...
fh.variables['T'].long_name		= 'Time mean meridional mean air temperature'
