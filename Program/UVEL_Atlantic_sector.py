#Program determines the zonal mean zonal velocity in the Atlantic sector
...
def ReadinData(filename):
	...
	U   	= fh.variables['U'][0, :, :] 		#Zonal velocity (m/s)
	V 	= fh.variables['V'][0, :, :]
	UU   	= fh.variables['UU'][0, :, :] 		#Zonal velocity (m/s)
	VV 	= fh.variables['VV'][0, :, :]
...
#	temp		= temp[:, :, lon_min_index:lon_max_index]
	...
	return lon, lat, lat_weight, hy_am, hy_bm, hy_ai, hy_bi, pres, u_vel,v_vel, temp
...
files = glob.glob(directory_data_1500+'*.cam2.h0.*.nc')
files.sort()
...
lon, lat, lat_weight, hy_am, hy_bm, hy_ai, hy_bi, pres, u_vel, v_vel, temp = ReadinData(files[0])
...
time_month	= time
u_vel_all	= ma.masked_all((len(time_year), len(P_lev), len(lat), len(lon)))
v_vel_all	= ma.masked_all((len(time_year), len(P_lev), len(lat), len(lon)))
uu_vel_all 	= ma.masked_all((len(time_year), len(P_lev), len(lat), len(lon)))
vv_vel_all 	= ma.masked_all((len(time_year), len(P_lev), len(lat), len(lon)))
u_vel_month 	= ma.masked_all((len(time_month), len(P_lev), len(lat)))
#u_vel_mean 	= ma.masked_all((len(time_year), len(P_lev)))
temp_all	= ma.masked_all((len(time_year), len(P_lev), len(lat)))
...
for year_i in range(int(np.min(time)), int(np.min(time))+len(time_year)):
	...
	vv_vel_year             = ma.masked_all((month_end-month_start+1, len(P_lev), len(lat), len(lon)))
	uu_vel_year             = ma.masked_all((month_end-month_start+1, len(P_lev), len(lat), len(lon)))
	...
	temp_year		= ma.masked_all((month_end-month_start+1, len(P_lev), len(lat), len(lon)))
	...
	for month_i in range(month_start, month_end+1):
		...
		lon, lat, lat_weight, hy_am, hy_bm, hy_ai, hy_bi, pres, u_vel, v_vel, temp	= ReadinData(filename)
		...
		for lat_i in range(len(lat)):
			for lon_i in range(len(lon)):
				...
				#print(month_i - month_start)
                    		...
                    		uu_vel_year[month_i-month_start, :len(u_vel_int), lat_i, lon_i]  = u_vel_int*u_vel_int
				vv_vel_year[month_i-month_start, :len(v_vel_int), lat_i, lon_i]  = v_vel_int*v_vel_int 
				
				#Potential temperature and interpolate
				pot_temp		= temp[:, lat_i, lon_i] * (1000. / P_3[:, lat_i, lon_i])**(0.286)
				temp_int		= interp1d(x,pot_temp)(x_int)
					
				#Save potential temperature field		
				temp_year[month_i-month_start, :len(temp_int), lat_i, lon_i]	= temp_int
		
		#Take zonal mean		
		u_vel_month[(year_i - int(np.min(time)))*12 + (month_i-month_start),:,:] = np.ma.mean(u_vel_year[month_i-month_start, :, :, :], axis=2)
	...
	uu_vel_year	= np.sum(uu_vel_year * month_days_all, axis = 0)
	vv_vel_year	= np.sum(vv_vel_year * month_days_all, axis = 0)

	u_vel_all[year_i - int(np.min(time))]	= u_vel_year
	v_vel_all[year_i - int(np.min(time))]   = v_vel_year
	vv_vel_all[year_i - int(np.min(time))]   = vv_vel_year
	uu_vel_all[year_i - int(np.min(time))]   = uu_vel_year
	#Determine the zonal mean
	#u_vel_all[year_i - int(np.min(time))]	= np.mean(u_vel_year, axis = 2)
	#temp_all[year_i - int(np.min(time))]	= np.mean(temp_year, axis = 2)
...
fh = netcdf.Dataset(directory+'Atmosphere/UVEL_VVEL_month_'+str(month_start)+'-'+str(month_end)+'_branch1500_year_'+str(year_start)+'_'+str(year_end)+'.nc', 'w')
...
fh.createVariable('U', float, ('time', 'lev', 'lat', 'lon'), zlib=True)
...
fh.createVariable('V', float, ('time', 'lev', 'lat', 'lon'), zlib=True)
fh.createVariable('UU', float, ('time', 'lev', 'lat', 'lon'), zlib=True)
fh.createVariable('VV', float, ('time', 'lev', 'lat', 'lon'), zlib=True)
