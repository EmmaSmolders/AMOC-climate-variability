#Program determines the velocities over the North Atlantic
...
def ReadinData(filename):
	...
	lon_2, U	= ConverterField3D(lon, U)
	lon_2, V	= ConverterField3D(lon, V)
	lon_2, UU	= ConverterField3D(lon, UU)
	lon_2, VV	= ConverterField3D(lon, VV)
	lon, pres	= ConverterField2D(lon, pres)
	
	#print(lon)
	
	#Atlantic sector -80 to 15
	lat_min_index	= (np.abs(lat - 0)).argmin()
	lat_max_index	= (np.abs(lat - 90)).argmin()+1
	lon_min_index	= (np.abs(lon - -90)).argmin()
	lon_max_index	= (np.abs(lon - 40)).argmin()+1
	...
	U		= U[:, lat_min_index:lat_max_index, lon_min_index:lon_max_index]
	V		= V[:, lat_min_index:lat_max_index, lon_min_index:lon_max_index]
	UU		= UU[:, lat_min_index:lat_max_index, lon_min_index:lon_max_index]
	VV		= VV[:, lat_min_index:lat_max_index, lon_min_index:lon_max_index]
...
for year_i in range(int(np.min(time)), int(np.min(time))+len(time_year)):
	...
	for month_i in range(month_start, month_end+1):
		...
		for lat_i in range(len(lat)):
			for lon_i in range(len(lon)):
				...
				uu_vel_year[month_i-month_start, :len(uu_vel_int), lat_i, lon_i]  = uu_vel_int
				vv_vel_year[month_i-month_start, :len(vv_vel_int), lat_i, lon_i]  = vv_vel_int
				
	#plt.figure()
	#plt.contourf(lon, lat, u_vel_year[0,0,:,:])
	#plt.show()
	...
	u_vel_all[year_i - int(np.min(time))]	= np.sum(u_vel_year * month_days_all, axis = 0)
	v_vel_all[year_i - int(np.min(time))]	= np.sum(v_vel_year * month_days_all, axis = 0)
	uu_vel_all[year_i - int(np.min(time))]	= np.sum(uu_vel_year * month_days_all, axis = 0)
	vv_vel_all[year_i - int(np.min(time))]	= np.sum(v_vel_year * month_days_all, axis = 0)
	
	plt.figure()
	plt.contourf(lon, lat, u_vel_all[0,0,:,:])
	plt.show()
...
fh = netcdf.Dataset(directory+'Atmosphere/Jet_Atlantic_month_'+str(month_start)+'-'+str(month_end)+'_branch1500_year_'+str(year_start)+'_'+str(year_end)+'.nc', 'w')
