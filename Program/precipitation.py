#Program retrieves precipitation
...
def ReadinData(filename):
	...
	lon, PREC	= ConverterField2D(lon, PREC)
	...
	#PREC		= PREC[lat_min_index:lat_max_index, lon_min_index:lon_max_index]
...
lon, lat, PREC = ReadinData(files[0])
...
PREC_all			= ma.masked_all((len(time_year), len(lat), len(lon)))
...
for year_i in range(int(np.min(time)), int(np.min(time))+len(time_year)):
	...
	PREC_year		= ma.masked_all((month_end-month_start+1, len(lat), len(lon)))
	...
	for month_i in range(month_start, month_end+1):
		...
		lon, lat, PREC	= ReadinData(filename)
		
		PREC_year[month_i - month_start, :, :] = PREC
	...
	for month_i in range(len(month_days)):
		...
		month_days_all[month_i]		= ma.masked_array(month_days_all[month_i], mask = PREC_year[month_i].mask)
	...
	PREC_year	= np.sum(PREC_year * month_days_all, axis = 0)

	#
	PREC_all[year_i - int(np.min(time))]	= PREC_year

#-----------------------------------------------------------------------------------------

print('Data is written to file')
fh = netcdf.Dataset(directory+'Atmosphere/PREC_month_'+str(month_start)+'-'+str(month_end)+'_branch3800_year_'+str(year_start)+'-'+str(year_end)+'.nc', 'w')
...
fh.createVariable('PREC', float, ('time', 'lat', 'lon'), zlib=True)

fh.variables['lat'].long_name		= 'Array of latitudes'
fh.variables['lon'].long_name		= 'Array of longitudes'
fh.variables['PREC'].long_name		= 'Time mean total precipitation'

fh.variables['time'].uniPREC 		= 'Model year'
fh.variables['lat'].uniPREC 		= 'Degrees N'
fh.variables['lon'].uniPREC 		= 'Degrees E'
fh.variables['PREC'].uniPREC 		= 'mm / day'

#Writing data to correct variable
fh.variables['time'][:] 		= time_year
fh.variables['lat'][:] 			= lat
fh.variables['lon'][:] 			= lon
fh.variables['PREC'][:] 		= PREC_all
