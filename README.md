...
The processed model output are stored as NETCDF files and using the relevant scripts one can regenerate the figures. Some scripts (e.g., EOF_AMV.py) use the original model output (not given here), but the generated time series are already available in the relevant directories.
